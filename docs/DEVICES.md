...
We will define these structures in `crates/gcodekit4-settings/src/devices.rs`.
...
The UI is implemented in `crates/gcodekit4-devicedb/ui/device_manager.slint` and integrated into the main application as a dedicated tab.
...
### Phase 1: Backend (`gcodekit4-devicedb`)
1.  Created `gcodekit4-devicedb` crate.
2.  Implemented `DeviceManager` and `DeviceProfile` models.
3.  Implemented persistence to `devices.json`.

### Phase 2: UI Implementation (`gcodekit4-devicedb` & `gcodekit4-ui`)
...
The `gcodekit4-devicedb` crate exposes the following public interfaces:
