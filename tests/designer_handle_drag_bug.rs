...
fn test_sequential_handle_drags() {
    ...
    if let Some(selected_id) = canvas.selected_id() {
        for obj in canvas.shapes() {
            if obj.id == selected_id {
                let (x1, y1, x2, y2) = obj.shape.bounding_box();
                assert!((x1 - 110.0).abs() < 0.1, "After 2nd drag: x1 should be 110, got {}", x1);
                assert!((y1 - 110.0).abs() < 0.1, "After 2nd drag: y1 should be 110, got {}", y1);
                assert!((x2 - 210.0).abs() < 0.1, "After 2nd drag: x2 should be 210, got {}", x2);
                assert!((y2 - 210.0).abs() < 0.1, "After 2nd drag: y2 should be 210, got {}", y2);
            }
        }
    }
}
...
fn test_alternating_corner_drags_no_jump() {
    ...
    if let Some(selected_id) = canvas.selected_id() {
        for obj in canvas.shapes() {
            if obj.id == selected_id {
                let (x1, y1, x2, y2) = obj.shape.bounding_box();
                assert!((x1 - 105.0).abs() < 0.1, "No jump: x1 should be 105, got {}", x1);
                assert!((y1 - 105.0).abs() < 0.1, "No jump: y1 should be 105, got {}", y1);
                assert!((x2 - 205.0).abs() < 0.1, "No jump: x2 should be 205, got {}", x2);
                assert!((y2 - 205.0).abs() < 0.1, "No jump: y2 should be 205, got {}", y2);
            }
        }
    }
}
