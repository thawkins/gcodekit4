...
fn test_designer_workflow_rectangle() {
    ...
    let selected = canvas.select_at(&select_point);
...
fn test_canvas_multi_shapes() {
    ...
    // Select each shape
    assert_eq!(canvas.select_at(&Point::new(5.0, 5.0)), Some(rect_id));
    assert_eq!(canvas.select_at(&Point::new(25.0, 25.0)), Some(circle_id));
    assert_eq!(canvas.select_at(&Point::new(50.0, 25.0)), Some(line_id));
