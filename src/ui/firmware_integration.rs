...
use crate::ui::settings_dialog::{SettingsDialog, SettingValue, SettingsCategory, Setting};
use crate::ui::firmware_settings_panel::{FirmwareSettingsPanel, FirmwareParameter, ParameterType};
...
impl FirmwareSettingsIntegration {
    ...
    pub fn load_grbl_defaults(&mut self) -> Result<()> {
        ...
        self.is_loaded = true;
        info!("Loaded {} GRBL firmware settings", self.firmware_panel.list_parameters().len());
        Ok(())
    }
    ...
    pub fn populate_dialog(&mut self, dialog: &mut SettingsDialog) {
        ...
        info!("Populating settings dialog with {} firmware parameters", 
        ...
        for param in params {
            ...
            let setting_value = match param.param_type {
                ...
                ParameterType::Boolean => {
                    let bool_val = matches!(value_str.to_lowercase().as_str(), "1" | "true" | "yes");
            ...
            self.cached_parameters.insert(param.code.clone(), param.clone());
        }
    ...
    pub fn update_from_dialog(&mut self, dialog: &SettingsDialog) -> Result<()> {
        ...
        let params_to_update: Vec<(String, String)> = self.firmware_panel
            .list_parameters()
            .iter()
            .filter_map(|param| {
                let setting_id = format!("fw_{}", param.code);
                dialog.get_setting(&setting_id).map(|setting| {
                    (param.code.clone(), setting.value.as_str().to_string())
                })
            })
            .collect();
        
        for (param_code, new_value) in params_to_update {
            match self.firmware_panel.set_parameter_value(&param_code, new_value.clone()) {
                Ok(_) => {
    ...
    pub fn restore_from_backup(&mut self) -> Result<()> {
        ...
        self.firmware_panel.restore_backup()
...
mod tests {
    ...
    fn test_modified_parameters() {
        ...
        // Modify a parameter
        let _ = integration.firmware_panel.set_parameter_value("$100", "500.0");
