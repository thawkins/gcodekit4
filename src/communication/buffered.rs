...
impl BufferedCommunicatorWrapper {
    ...
    pub fn queue_command(&self, command: String) -> crate::Result<()> {
        let mut queue = self
            .command_queue
            .lock()
            .map_err(|e| crate::Error::other(format!("Failed to lock command queue: {}", e)))?;

        if queue.len() >= self.config.queue_size {
            return Err(crate::Error::other("Command queue is full"));
        }
    ...
    pub fn queued_commands_count(&self) -> crate::Result<usize> {
        let queue = self
            .command_queue
            .lock()
            .map_err(|e| crate::Error::other(format!("Failed to lock command queue: {}", e)))?;
    ...
    pub fn active_commands_count(&self) -> crate::Result<usize> {
        let active = self
            .active_commands
            .lock()
            .map_err(|e| crate::Error::other(format!("Failed to lock active commands: {}", e)))?;
        Ok(active.len())
    }
    ...
    pub fn stream_commands(&mut self) -> crate::Result<()> {
        if self.send_paused {
            return Ok(());
        }
        ...
        loop {
            ...
            if let Some(mut command) = queue.pop_front() {
                ...
                let mut active = self.active_commands.lock().map_err(|e| {
                    crate::Error::other(format!("Failed to lock active commands: {}", e))
                })?;
                active.push(command);
            } else {
                break;
    ...
    pub fn handle_acknowledgment(&mut self) -> crate::Result<()> {
        let mut active = self
            .active_commands
            .lock()
            .map_err(|e| crate::Error::other(format!("Failed to lock active commands: {}", e)))?;
    ...
    pub fn handle_error(&mut self, error_msg: String) -> crate::Result<()> {
        let mut active = self
            .active_commands
            .lock()
            .map_err(|e| crate::Error::other(format!("Failed to lock active commands: {}", e)))?;
        ...
        if let Some(command) = active.first_mut() {
            ...
            if command.can_retry() {
                ...
                let mut queue = self.command_queue.lock().map_err(|e| {
                    crate::Error::other(format!("Failed to lock command queue: {}", e))
                })?;
    ...
    pub fn clear_queue(&mut self) -> crate::Result<()> {
        let mut queue = self
            .command_queue
            .lock()
            .map_err(|e| crate::Error::other(format!("Failed to lock command queue: {}", e)))?;
