...
mod tests {
    ...
    fn test_buffer_status_parse() {
        let buffer = BufferStatus::parse("14,1").unwrap();
        assert_eq!(buffer.rx, 14);
        assert_eq!(buffer.tx, 1);
    }

    #[test]
    fn test_overrides_parse() {
        let ov = Overrides::parse("120,80,100").unwrap();
        assert_eq!(ov.feed_rate, 120);
        assert_eq!(ov.spindle_speed, 80);
        assert_eq!(ov.rapid, 100);
    }

    #[test]
    fn test_device_status_parse_full() {
        let response = "<Idle|MPos:10.000,20.000,0.000|WPos:10.000,20.000,0.000|Bf:14,1|Ov:120,80,100|F:2000|S:5000>";
        let status = DeviceStatus::parse_grbl_status(response).unwrap();

        assert_eq!(status.state, MachineStateType::Idle);
        assert_eq!(status.machine_pos, (10.0, 20.0, 0.0));
    ...
    fn test_device_status_parse_minimal() {
        let response = "<Idle|MPos:0.000,0.000,-5.000|WPos:0.000,0.000,0.000>";
        let status = DeviceStatus::parse_grbl_status(response).unwrap();

        assert_eq!(status.state, MachineStateType::Idle);
        assert_eq!(status.machine_pos, (0.0, 0.0, -5.0));
        assert_eq!(status.work_pos, (0.0, 0.0, 0.0));
        assert_eq!(status.buffer, None);
    ...
    fn test_running_state() {
        ...
        assert_eq!(status.spindle_speed, Some(12000));
    }
}
